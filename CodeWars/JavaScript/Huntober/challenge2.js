...
function scrapStr(string) {
    let index = string.indexOf('c')
    string = string.split('').slice(index, string.length - 1).join('')
    string = string.replaceAll('Eek!', '').split('').reverse('').join('')
    // string = string.split('v').map(x => x[0])

    return string.split('').map(x => key.includes(x) ? ' ' : x).join('')
}

console.log(scrapStr(str))